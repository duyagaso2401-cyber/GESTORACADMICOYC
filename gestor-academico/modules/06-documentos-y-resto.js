...
let asistHoraFin=new Date(new Date().getTime() + 60*60000).toTimeString().slice(0,5);
let asistPeriodo='P1';
let asistGrado='';
let asistCId='';
...
function htmlAsistencia(){
  ...
  html+='<div><label class="lbl">Hora inicio</label><input type="time" id="asistHoraInp" value="'+asistHora+'" onchange="asistHora=this.value"></div>';
  ...
  if(ests.length){
    ...
    html+=_htmlEstadoVacio('🎓','Seleccione grado y asignatura para ver los estudiantes.');
  }
  html+='</div></div>';
  ...
  if(historial.length){
    ...
    html+=_htmlEstadoVacio('🗓️','No hay registros de asistencia aún para los filtros seleccionados.');
  }
  html+='</div></div>';
  ...
  if(isAdmin){
    ...
    html+='</div><div id="reporteAsistWrap">'+_htmlEstadoVacio('🔎','Seleccione filtros para ver el consolidado.')+'</div></div></div>';
  }
...
async function abrirConfigInasistenciasRapida(){
  var pctCrit = Number(db.config?.pctInasistenciaCritica || 25);
  var pctPrev = Number(db.config?.pctInasistenciaPreventiva || 20);

  var nCrit = await customPrompt('🔴 Ingrese el porcentaje crítico de inasistencia para PÉRDIDA/REPROBACIÓN (ej: 25):', pctCrit);
  ...
  var nPrev = await customPrompt('🟡 Ingrese el porcentaje preventivo de ALERTA TEMPRANA (ej: 20):', pctPrev);
      ...
      ausentes.forEach(function(estId){
        ...
        try{
          fetch(notifEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({sk:_skActual(),kind:'ausencia',actor:sesion.n,message:msgNotif,
...
function verReporteAsistencia(){
  ...
  if(!ests.length||!regs.length){wrap.innerHTML=_htmlEstadoVacio('📭','No hay datos.');return;}
...
function descargarPlanillaAsistenciaPDF(){
  ...
  var escH=34,escW=28;
  // Escudo institución (izquierda)
  if(escLogoAsist){try{doc.addImage(escLogoAsist,'PNG',ML+1,3,escW,escH);}catch(e){}}
  // Escudo Colombia (derecha)
  if(escColAsist){try{doc.addImage(escColAsist,'PNG',W-MR-escW-1,3,escW,escH);}catch(e){}}
  ...
  // Nombre institución
  var instNom=(db.nombre||'INSTITUCIÓN EDUCATIVA').toUpperCase();
...
function descargarReporteAsistenciaPDF(){
  ...
  doc.text(db.nombre||'INSTITUCIÓN EDUCATIVA',105,10,{align:'center'});
...
function htmlCentrosInteres(){
  ...
  return '<h3 class="sec-title">&#x1F3AF; Centros de Interes Institucional</h3>'+
    ...
    (centros.length?('<h3 class="sec-title">Centros Registrados ('+centros.length+')</h3>'+cardsHtml):
      '<div class="card">'+_htmlEstadoVacio('📭','No hay centros de interes registrados aun.')+'</div>');
}
function crearCI(){
...
async function editarCI(i){
  var ci=(db.centrosInteres||[])[i];if(!ci) return;
  var nn=await customPrompt('Nombre:',ci.nombre);if(nn===null) return;
  var nr=await customPrompt('Responsable:',ci.responsable);if(nr===null) return;
  var nd=await customPrompt('Descripcion:',ci.descripcion||'');
  var nfe=await customPrompt('Fecha entrega (AAAA-MM-DD):',ci.fechaEntrega||'');
  updDB(function(d){
...
function enviarMensajeRectora(){
  ...
  fetch(API_BASE+'/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({sk:_skActual(),kind:'mensaje',actor:sesion.n,message:'Mensaje de '+sesion.n+': '+txt,meta:{u:sesion.u}})})
...
function cargarNotificaciones(){
  ...
  fetch(API_BASE+'/api/inetis/notifications?sk='+encodeURIComponent(_skActual()||'')+'&limit=100').then(r=>r.json()).then(j=>{
    const list=j.notifications||[];
    if(!list.length){wrap.innerHTML=_htmlEstadoVacio('🔔','Sin notificaciones aún.');return;}
    wrap.innerHTML='<div class="over"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>De</th><th>Mensaje</th><th>Estado</th></tr></thead><tbody>'+
      list.map(n=>{
        const f=new Date(n.createdAt).toLocaleString('es-CO');
  ...
  }).catch(()=>{wrap.innerHTML=_htmlEstadoVacio('📡','Sin conexión al servidor.');});
}
function marcarLeidasTodas(){
  fetch(API_BASE+'/api/inetis/notify/seen',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual()})})
...
function renderPadre(){
  ...
  const obsHtmlPadre=obsEst.length?obsEst.map(o=>{
    ...
    return `<div style="border-left:4px solid ${col};background:${bg};padding:12px 16px;border-radius:0 10px 10px 0;margin-bottom:10px">
      ...
      <p style="margin:0;font-size:0.88rem;color:var(--text-secondary);line-height:1.5">${o.txt||''}</p>
    </div>`;
  ...
  <div style="max-width:960px;margin:18px auto;padding:0 14px">
    ...
    <div style="background:#fff;border:1px solid #d5ead8;border-radius:12px;padding:16px 18px;margin-bottom:14px;box-shadow:0 1px 5px rgba(0,0,0,0.05)" id="padre-tablon-card">
      ...
      <div id="padre-tablon-list"><div style="text-align:center;padding:16px;color:#aaa;font-size:0.85rem">⏳ Cargando anuncios...</div></div>
    </div>
  ...
  setTimeout(()=>_cargarTablonEst(est.g,'padre-tablon-list',est.id),120);
}
...
function htmlAsistEstudiante(est){
  ...
  if(!regs.length) return _htmlEstadoVacio('👨‍🏫','Sin registros de asistencia registrados por el docente aún.');
...
// ============================================================
// ============================================================
// TABLÓN DE ANUNCIOS (padre/estudiante) — vista simple, separada
// de "Mis Comunicados", con solo los anuncios de tipo aviso-docente
// para el grado del estudiante. No es un chat: solo lectura.
// ============================================================
async function _cargarTablonEst(grado, listId, estId){
  const cont=document.getElementById(listId);
  if(!cont) return;
  cont.innerHTML='<div style="text-align:center;padding:14px;color:#aaa;font-size:0.85rem">⏳ Cargando anuncios...</div>';
  try{
    const r=await fetch('/api/inetis/notifications?sk='+encodeURIComponent(_skActual()||''));
    if(!r.ok) throw new Error('HTTP '+r.status);
    const j=await r.json();
    const all=(j.notifications||[]);
    const platIdActual=window._currentPlatId||null;
    const avisos=all.filter(n=>{
      if(n.kind!=='aviso-docente') return false;
      const m=n.meta||{};
      if(m.platId&&platIdActual&&m.platId!==platIdActual) return false;
      if(m.grado&&grado&&m.grado!==grado) return false; // sin grado = para todos sus grupos
      return true;
    }).sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0)).slice(0,30);
    if(!avisos.length){
      cont.innerHTML='<div style="text-align:center;padding:18px"><span style="font-size:1.8rem">📭</span><p style="color:#aaa;font-size:0.85rem;margin-top:6px">Sin anuncios publicados para su grado todavía.</p></div>';
      return;
    }
    // Confirmación de lectura: se marca localmente (dentro de la BD de la
    // institución, se sincroniza como todo lo demás) que este estudiante ya
    // vio estos anuncios, para que el docente vea qué porcentaje los leyó.
    if(estId&&avisos.length&&!(sesion&&sesion.soloLectura)){
      updDB(function(d){
        d.avisosLeidos=d.avisosLeidos||{};
        avisos.forEach(function(n){
          const key=String(n.id);
          if(!d.avisosLeidos[key]) d.avisosLeidos[key]=[];
          if(!d.avisosLeidos[key].includes(String(estId))) d.avisosLeidos[key].push(String(estId));
        });
        return d;
      });
    }
    cont.innerHTML=avisos.map(n=>{
      const m=n.meta||{};
      const f=n.createdAt?new Date(n.createdAt).toLocaleString('es-CO',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}):'';
      let cuerpo=n.message||'';
      if(m.asunto){
        const prefijo=m.asunto+'\n\n';
      ...
      return `<div style="border-left:4px solid #1e8449;background:#eafaf1;padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:10px">
        <div style="display:flex;align-items:center;flex-wrap:wrap;gap:6px;margin-bottom:4px">
          ...
          <span style="font-size:0.72rem;color:#aaa;margin-left:auto">${f}</span>
        </div>
        <p style="margin:0 0 4px;font-weight:700;font-size:0.87rem;color:var(--text-secondary)">${m.asunto||'(sin asunto)'}</p>
        ${cuerpo?`<p style="margin:0 0 4px;font-size:0.83rem;color:var(--text-secondary);line-height:1.5;white-space:pre-wrap">${cuerpo}</p>`:''}
        <p style="margin:0;font-size:0.72rem;color:#aaa">Publicado por: <b>${m.docente||n.actor||'Docente'}</b></p>
      </div>`;
    }).join('');
  }catch(e){
    cont.innerHTML='<div style="padding:14px;color:#c0392b;font-size:0.85rem;text-align:center">❌ No se pudo cargar el tablón. Verifique su conexión.</div>';
  }
}
  ...
  try{
    ...
    cont.innerHTML=related.slice(0,40).map(n=>{
      ...
      return `<div style="border-left:4px solid ${ki.color};background:${n.seen?'#fafafa':ki.bg};padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:10px;transition:background 0.3s">
        ...
        <p style="margin:0 0 4px;font-size:0.87rem;color:var(--text-secondary);line-height:1.55">${n.message||''}</p>
...
function renderEstudiante(){
  ...
  const obsHtml=obsEst.length?obsEst.map(o=>{
    ...
    return `<div style="border-left:4px solid ${col};background:${bg};padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:8px">
      ...
      <p style="margin:4px 0;font-size:0.86rem;color:var(--text-secondary)">${o.txt||''}</p>
    </div>`;
  }).join(''):`"+_htmlEstadoVacio('📝','No hay anotaciones en el observador hasta el momento.')}`;
  ...
  <div class="main">
    ...
    ${!_pushYaActivado()?`<div class="card" style="border-left:4px solid #f1c40f;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px">
      <div style="font-size:0.85rem;color:#7d6608"><b>🔔 Active las notificaciones</b><br>Entérese al instante de anuncios y avisos, incluso con la app cerrada.</div>
      <button class="btn" style="background:#b7950b;color:#fff" onclick="activarNotificacionesPush()">🔔 Activar</button>
    </div>`:''}
    ${htmlEleccionEstudiante(est)}
    <div class="card" id="est-tablon-card">
      <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:12px">
        <h4 class="card-title" style="margin:0;color:#1e8449">📢 Tablón de Anuncios — Grado ${est.g}</h4>
        <button class="btn" style="background:#1e8449;color:#fff;font-size:0.78rem;padding:5px 12px" onclick="_cargarTablonEst(${JSON.stringify(est.g)},'est-tablon-list',${JSON.stringify(est.id)})">🔄 Actualizar</button>
      </div>
      <div id="est-tablon-list"><div style="text-align:center;padding:14px;color:#aaa;font-size:0.85rem">⏳ Cargando anuncios...</div></div>
    </div>
    ...
    <div class="card"><h4 class="card-title">📚 Material y Tareas Recibidas</h4>
      ${trabsEst.length?'<ul style="padding-left:16px">'+trabsEst.map(t=>'<li style="margin-bottom:6px"><b>'+(t.titulo||t.nombre||'Material')+'</b> <span style="color:#888;font-size:0.8rem">· '+(t.fecha||'')+'</span>'+(t.url?' <a href="'+t.url+'" target="_blank" style="color:#1a5276">📎 Ver archivo</a>':'')+'</li>').join('')+'</ul>':_htmlEstadoVacio('📚','Sin material recibido aún.')}
    </div>
  ...
  setTimeout(()=>_cargarTablonEst(est.g,'est-tablon-list',est.id),120);
}
...
function _verObservadorCompleto(estId){
  ...
  if(!obs.length){h+=_htmlEstadoVacio('📝','No hay anotaciones registradas.');}
  else{
    ...
    try{
      ...
      updDB(d=>{
        for(let i=hdrIdx+1;i<rows.length;i++){
          ...
          // Evitar duplicados por número de documento: se valida en TODA la
          // institución (sin importar el grado), porque un mismo documento
          // no debería repetirse entre grados — evita crear un registro
          // duplicado si el estudiante ya fue matriculado, o si el archivo
          // se vuelve a importar por error. El nombre solo se compara dentro
          // del mismo grado, como respaldo cuando no hay número de documento.
          const existe=d.ests.some(e=>(numDoc&&e.numDoc===numDoc)||(e.g===grado&&e.n===(ape+' '+nom).trim()));
  ...
  r.onload=ev=>{
    ...
    updDB(d=>{
      for(let i=start;i<lines.length;i++){
        ...
        // Misma validación de duplicados que el importador de Excel: por
        // número de documento en toda la institución, o por nombre dentro
        // del mismo grado si no hay número de documento.
        const existe=d.ests.some(e=>(numDoc&&e.numDoc===numDoc)||(e.g===grado&&e.n===(ape+' '+nom).trim()));
        if(existe){omitidos++;continue;}
        if(!d.grados.some(g=>g.n===grado)){d.grados.push({n:grado,d:''});}
    ...
    const st=document.getElementById('masivoStatus');
    if(st){st.innerHTML='✅ '+creados+' estudiantes creados.'+(omitidos?' | ⏭️ '+omitidos+' ya existían (omitidos)':'')+(errores.length?'<br><span style=color:#c0392b>⚠ '+errores.length+' errores: '+errores.slice(0,5).join(' · ')+'</span>':'');}
...
function vistaPreviaPromocion(){
  ...
  if(!ests.length){wrap.innerHTML=_htmlEstadoVacio('🎓','No hay estudiantes registrados.');return;}
  let rows='';
  ests.forEach(e=>{
...
function htmlTablero(){
  ...
  const porGrado=grados.map(g=>{
    ...
    const matsG=_cargaDelGrado(g.n);
...
function _htmlBIDirectivo(ests,grados,carga,asistencia){
  ...
  <div style="display:grid;grid-template-columns:3fr 2fr;gap:14px;margin-bottom:14px;flex-wrap:wrap">
    <div class="card">
      <h4 style="color:var(--text-secondary);margin:0 0 12px;font-size:0.93rem">📋 Embudo de Conversión — Pre-Matrículas</h4>
      ${pmTotal===0?`${_htmlEstadoVacio('📝','Sin solicitudes de pre-matrícula registradas.')}`:`
  ...
  <div style="display:grid;grid-template-columns:3fr 2fr;gap:14px;margin-bottom:14px">
    ...
    <div class="card">
      ...
      <div style="max-height:320px;overflow-y:auto">${topGradoHtml||_htmlEstadoVacio('📝','Sin datos de notas aún.')}</div>
    </div>
  </div>
...
async function enviarComunicadoGeneral(){
  ...
  try{
    ...
    const r=await fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual(),kind:tipo,actor,message:mensajeCompleto,meta})});
    ok=r.ok;
  ...
  if(envEmail&&ok){
    ...
    for(let i=0;i<conEmail.length;i++){
      ...
      try{
        ...
        const r2=await fetch('/api/inetis/send-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:e.email,subject:`[${db.nombre||'Institución'}] ${tipoLabel}${gradoSufijo}: ${asunto}`,text:bodyEmail})});
  ...
  histItems.innerHTML=_comHistorial.map(h=>`<div style="border-left:3px solid ${h.ok?'#27ae60':'#e74c3c'};padding:7px 12px;margin-bottom:8px;background:#fff;border-radius:0 6px 6px 0">
    ...
    <p style="margin:4px 0 0;font-size:0.83rem;color:var(--text-secondary);font-weight:600">${h.asunto}</p>
  </div>`).join('');
  if(ok){
...
function htmlAvisoDocente(){
  ...
  return `<h3 class="sec-title">📢 Tablón de Anuncios</h3>
  <div class="info-box" style="margin-bottom:14px">
    <b>Sus grados asignados:</b> ${misGrados.join(', ')} · <b>Publique anuncios, tareas o recordatorios</b> por grado y/o asignatura. Aparecerán en la sección <b>"📬 Mis Comunicados"</b> del portal del estudiante y del acudiente.
  </div>
...
async function enviarAvisoDocente(){
  ...
  let ok=false;
  try{
    const meta={grado:grado||null,asunto,asig:asig||null,fechaLimite:fecha||null,docente:sesion.n,docenteU:sesion.u,platId:window._currentPlatId||null,platSK:window._currentPlatSK||null};
    const r=await fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual(),kind:tipo,actor,message:mensajeCompleto,meta})});
    ok=r.ok;
    barra.style.width='100%';
    if(ok){
  ...
  if(ok){
    ...
    setTimeout(()=>{progreso.style.display='none';barra.style.width='0%';},4000);
    _cargarTablonDocente();
  } else {
    logDiv.innerHTML='❌ Error al publicar el aviso ('+ahora+'). Intente nuevamente.';
  }
}

// ============================================================
// TABLÓN DE ANUNCIOS (docente) — historial real y persistente de
// los avisos publicados por este docente, no solo los de la
// sesión actual. Se apoya en el mismo endpoint que ya usa el
// portal del acudiente/estudiante para mostrar comunicados.
// ============================================================
async function _cargarTablonDocente(){
  const histDiv=document.getElementById('docComHistorial');
  const histItems=document.getElementById('docComHistItems');
  if(!histDiv||!histItems) return;
  histDiv.style.display='block';
  histItems.innerHTML='<div style="text-align:center;padding:12px;color:#aaa;font-size:0.85rem">⏳ Cargando anuncios publicados...</div>';
  try{
    const r=await fetch('/api/inetis/notifications?sk='+encodeURIComponent(_skActual()||''));
    if(!r.ok) throw new Error('HTTP '+r.status);
    const j=await r.json();
    const all=(j.notifications||[]);
    const platIdActual=window._currentPlatId||null;
    const mios=all.filter(n=>{
      if(n.kind!=='aviso-docente') return false;
      const m=n.meta||{};
      if(m.docenteU!==sesion.u) return false;
      // Anuncios nuevos ya guardan platId; los antiguos (antes de esta mejora) no lo tienen y se muestran igual.
      if(m.platId&&platIdActual&&m.platId!==platIdActual) return false;
      return true;
    }).sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));
    if(!mios.length){
      histItems.innerHTML='<p class="empty" style="padding:10px">Aún no ha publicado anuncios. Los que publique aparecerán aquí, incluso en sus próximas sesiones.</p>';
      return;
    }
    histItems.innerHTML=mios.map(n=>{
      const m=n.meta||{};
      const f=n.createdAt?new Date(n.createdAt).toLocaleString('es-CO',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'}):'';
      // Confirmación de lectura: cuántos de los estudiantes del grado ya
      // vieron este anuncio (db.avisosLeidos se llena desde el portal del
      // estudiante/acudiente cada vez que lo consultan).
      const totalGrado=m.grado?(db.ests||[]).filter(function(e){return e.g===m.grado&&!e.deletedAt;}).length:(db.ests||[]).filter(function(e){return !e.deletedAt;}).length;
      ...
      return `<div style="border-left:3px solid #1e8449;padding:7px 12px;margin-bottom:8px;background:#fff;border-radius:0 6px 6px 0">
        ...
        ${m.fechaLimite?`<span style="font-size:0.72rem;background:#fde8e8;color:#c0392b;border-radius:4px;padding:1px 7px;margin-left:4px">📅 ${m.fechaLimite}</span>`:''}
        <span style="font-size:0.72rem;background:${pctLeido>=70?'#eafaf1':pctLeido>=30?'#fef9e7':'#fdedec'};color:${pctLeido>=70?'#1e8449':pctLeido>=30?'#7d6608':'#c0392b'};border-radius:4px;padding:1px 7px;margin-left:4px" title="${leidos} de ${totalGrado} estudiante(s) lo han visto">👁 ${pctLeido}% leído</span>
        <p style="margin:4px 0 0;font-size:0.84rem;color:var(--text-secondary);font-weight:600">${m.asunto||'(sin asunto)'}</p>
      </div>`;
    }).join('');
  }catch(e){
    histItems.innerHTML='<p class="empty" style="padding:10px;color:#c0392b">No se pudo cargar el tablón. Verifique su conexión.</p>';
  }
}
...
async function enviarAlertasSeleccionadas(){
  ...
  for(const idx of selIdxs){
    ...
    try{
      ...
      await fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({sk:_skActual(),kind:'alerta-academica',actor:sesion.n||'Sistema',message:msgN,
      ...
      if(c.email){
        ...
        const resp=await fetch('/api/inetis/send-email',{method:'POST',headers:{'Content-Type':'application/json'},
...
function htmlEstadoNotas(){
  ...
  if(!misCargas.length) return '<h3 class="sec-title">🔍 Estado de Notas</h3><div class="card">'+_htmlEstadoVacio('📭','No tiene asignaturas asignadas.')+'</div>';
...
function cargarFotoPerfil(inp){
  ...
  fileToCloudinaryUrl(inp.files[0],function(url){
    if(!url){ if(prev) prev.innerHTML=sesion.foto?`<img src="${_cloudinaryThumb(sesion.foto,150)}" style="width:100%;height:100%;object-fit:cover">`:'<span style="font-size:2rem">👤</span>'; return; }
...
function htmlAusentismo(){
  ...
  <div class="info-box" style="margin-bottom:12px">Formulario oficial — Gobernación de Bolívar / Secretaría de Educación del Departamento de Bolívar.</div>
  <div class="card">
...
function enviarAusentismo(){
  ...
  try{fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual(),kind:'permiso-laboral',actor:sesion.n,message:sesion.n+' envió una solicitud de permiso laboral (H03.03.F01). Tipos: '+tipos.join(', ')+'.',meta:solicitud})}).catch(()=>{});}catch(e){}
  ...
  (async()=>{
    try{
      ...
      if(emailRector){
        ...
        await fetch('/api/inetis/send-email',{method:'POST',headers:{'Content-Type':'application/json'},
...
function imprimirPermisoH03(id){
  ...
  doc.text('DATOS DEL PERMISO (Máx 3 días hábiles consecutivos — Ley 734/2002 — Dec.1083/2015 — Dec.648/2017)',CX,y+0.5,{align:'center',maxWidth:W-28});y+=8;
  ...
  doc.text('Observaciones: '+sol.obs,14,y,{maxWidth:W-28});y+=10;
  doc.text('Soporte Anexo: '+sol.soporte,14,y,{maxWidth:W-28});y+=16;
  // Firmas
  ...
  if(sol.estado==='Aprobado'){
    ...
    doc.text('ESTA SECCIÓN DEBE SER ENTREGADA AL SOLICITANTE DEL PERMISO EN CASO DE SER APROBADO.',CX,y+2,{align:'center',maxWidth:W-28});y+=8;
...
function htmlManualUsuario(){
  ...
  const contenidos={
    ...
    admin:`
      ...
      ${section('⚙️ Configuración Institucional','#8e44ad',`
        ${card('🏫','Configuración Base','Menú lateral → 🏫 Configuración Base',[
          'Ingrese el <b>nombre completo</b> de la institución, municipio y departamento (Córdoba/Bolívar).',
      ...
      ${section('🆕 Funciones Recientes','#c0392b',`
        ${card('🎓','Notas de Actividades en Clase','Menú lateral → 📝 Notas de Actividades',[
          'Registre notas de talleres, actividades en clase, actividades en casa y participación, por asignatura y periodo.',
          'El promedio de estas actividades se sincroniza automáticamente con la <b>Planilla de Calificaciones</b> (columna HACER o SER, según la configuración).',
          'Si no ve este módulo en su menú, revise en el panel del súper admin (o pídale que revise) que el módulo "📝 Notas de Actividades en Clase" esté activado para su institución.',
        ],'Este módulo lleva tiempo funcionando junto a Planilla — si desaparece de su menú, generalmente es porque quedó desactivado por accidente al editar los módulos de la institución, no porque se haya eliminado.')}

        ${card('📅','Calendario Académico','Menú lateral → 📅 Calendario Académico',[
          'Vea en un solo calendario mensual: sus propios eventos (reuniones, entregas), los festivos colombianos (calculados automáticamente) y las fechas de apertura/cierre del Cronograma de Notas.',
          'Use <b>+ Agregar Evento</b> para registrar reuniones, entregas o avisos en una fecha específica, con grado opcional.',
          'La lista de <b>Próximos Eventos</b> le muestra de un vistazo todo lo que se viene.',
        ],'No reemplaza el Cronograma de Notas ni los Horarios — los reúne en una sola vista para no tener que revisar varios módulos por separado.')}

        ${card('📈','Comparativo entre Años','Menú lateral → 📅 Histórico de Años → sección "Comparativo entre Años"',[
          'Seleccione dos años (el actual y uno archivado) para comparar promedio y % de aprobación por grado.',
          'Necesita al menos un año archivado con el botón "📦 Archivar Año Actual" para poder comparar.',
          'Exporte el comparativo a PDF con el botón correspondiente.',
        ],'Muy útil para consejos académicos: muestra de un vistazo si un grado mejoró o empeoró respecto al año anterior.')}

        ${card('🔒','Coordinadores de Solo Lectura','Menú lateral → 🏫 Configuración Base (enlace) o 📚 Carga Académica',[
          'Permite delegar en un coordinador o auxiliar sin darle permiso de editar nada: ve todo el panel exactamente igual que el rector, pero no puede guardar, eliminar ni modificar nada.',
          'Cree el acceso indicando nombre, usuario y contraseña — queda con rol "admin" pero marcado como solo lectura.',
          'Si el coordinador ya no debe tener acceso, elimínelo desde la misma sección.',
        ],'Ideal para auditorías internas o para que un auxiliar consulte información sin riesgo de que modifique algo por error.')}

        ${card('🔍','Firma Digital y Verificación de Documentos','Menú lateral → 📄 Documentos/Actas → pestaña "🔍 Verificar Documento"',[
          'Cada boletín y acta que genere ahora incluye un <b>código de verificación</b> y un <b>código QR</b> al pie de la página, firmados por el servidor.',
          'Para verificar un documento: pegue el código de texto, o suba una foto/captura del código QR, y el sistema confirma si las notas, el rector y el director de grupo coinciden con lo que hay registrado actualmente.',
          'Si el documento fue alterado después de emitirlo, el sistema se lo advierte claramente.',
        ],'El código no se puede falsificar desde el navegador: se firma con una clave que solo conoce el servidor. Es un respaldo oficial ante reclamos de padres sobre boletines impresos.')}

        ${card('📜','Auditoría de Matrícula y Traslados','Menú lateral → 📜 Historial de Cambios → pestaña "🎓 Matrícula y Traslados"',[
          'Consulte quién trasladó a un estudiante de grado, cuándo, y qué cambió exactamente.',
          'También registra ediciones de nombre/grado hechas desde el perfil del estudiante.',
          'Exporte el reporte filtrado a PDF firmado con el botón "🖨️ Exportar PDF Firmado" — útil como respaldo oficial ante un reclamo de un padre.',
        ],'Funciona junto al historial de cambios en notas que ya existía, ahora con una pestaña dedicada a matrícula.')}

        ${card('⌨️','Atajos de Teclado en Planilla','Módulo Planilla, con el foco en una celda de nota',[
          'Use las <b>flechas del teclado</b> para moverse entre las celdas de notas sin usar el mouse.',
          'Presione <b>Enter</b> o <b>Espacio</b> sobre una celda para abrir el selector de nota.',
          'Dentro del selector, las <b>flechas</b> mueven el foco entre los valores y <b>Enter</b> confirma el elegido.',
        ],'Pensado para quien califica cientos de estudiantes: reduce mucho la dependencia del mouse.')}

        ${card('📴','Modo Offline / Guardado Automático','Automático, en cualquier módulo',[
          'Si se corta la conexión a internet mientras trabaja, sus cambios se siguen guardando localmente en el dispositivo — no se pierden.',
          'Un indicador "📴 Sin conexión — guardado local" aparece en la parte superior mientras no haya señal.',
          'Al recuperar la conexión, el sistema reintenta enviar los cambios pendientes automáticamente, sin que tenga que hacer nada.',
        ],'')}

        ${card('🔊','Lector de Pantalla y 🗣️ Navegación por Voz','Menú "⚙️ Acciones" (parte superior)',[
          '<b>Lector de Pantalla</b>: al activarlo, el sistema lee en voz alta el botón o campo que reciba el foco (con Tab) o el mouse — pensado para personas con baja visión.',
          '<b>Navegación por Voz</b>: es lo contrario — usted habla el nombre de un módulo (ej. "asistencia", "notas") y el sistema navega hacia allá.',
          'Ambas opciones están en el mismo menú desplegable "⚙️ Acciones", y funcionan de forma independiente.',
        ],'Requiere un navegador compatible (Chrome o Edge) para el reconocimiento y la síntesis de voz.')}

        ${card('🧭','Recorrido Guiado y Primeros Pasos','Módulo Tablero',[
          'Si es la primera vez que configura la institución, verá un checklist "🚀 Primeros Pasos" que le indica qué falta por hacer.',
          'El botón "🧭 Ver recorrido guiado" le muestra, paso a paso, dónde está cada función principal — resaltando en amarillo el botón del menú correspondiente.',
          'Puede repetir el recorrido cuando quiera desde el mismo botón.',
        ],'')}

        ${card('🌙','Modo Oscuro y 📱 Instalar como App','Barra lateral (Modo Oscuro) / navegador (Instalar App)',[
          'El botón <b>🌙 Modo Oscuro</b>, en la parte de abajo del menú lateral, cambia los colores de toda la interfaz para reducir el cansancio visual.',
          'El sistema es instalable como aplicación (PWA): en el navegador, busque la opción "Instalar" o "Agregar a pantalla de inicio" para tenerlo como una app normal en su celular o computador.',
        ],'')}
      `)}`,
    ...
    docente:`
      ...
      ${section('🆕 Funciones Recientes','#c0392b',`
        ${card('📢','Tablón de Anuncios','Menú lateral → 📢 Tablón de Anuncios',[
          'Publique avisos para sus estudiantes/acudientes de un grado específico: tarea, fecha de entrega, recordatorios.',
          'Cada aviso muestra un indicador "👁 X% leído" que le dice qué porcentaje de los estudiantes del grado ya lo consultaron.',
          'El historial de avisos publicados queda disponible para consultarlo luego.',
        ],'La confirmación de lectura se registra automáticamente cuando el acudiente o estudiante abre su Tablón — no requiere que hagan nada aparte.')}

        ${card('🎓','Notas de Actividades en Clase','Menú lateral → 📝 Notas de Actividades',[
          'Registre notas de talleres, actividades en clase, actividades en casa y participación.',
          'El promedio se sincroniza automáticamente con la <b>Planilla de Calificaciones</b>.',
          'Si no ve este módulo, pídale al rector que verifique que esté activado para la institución.',
        ],'')}

        ${card('⌨️','Atajos de Teclado en Planilla','Módulo Planilla, con el foco en una celda de nota',[
          'Use las <b>flechas</b> para moverse entre celdas sin el mouse.',
          'Presione <b>Enter</b> o <b>Espacio</b> para abrir el selector de nota, y de nuevo <b>Enter</b> para confirmar el valor elegido.',
        ],'Muy útil si califica muchos estudiantes seguidos.')}

        ${card('📴','Modo Offline / Guardado Automático','Automático, en cualquier módulo',[
          'Si pierde la conexión mientras califica, sus notas se siguen guardando localmente y se envían solas apenas vuelva la señal — no se pierden.',
          'Un aviso "📴 Sin conexión — guardado local" se lo indica mientras tanto.',
        ],'')}

        ${card('🌙','Modo Oscuro y 🔊 Lector de Pantalla','Barra lateral (Modo Oscuro) / menú "⚙️ Acciones" (Lector)',[
          'Active el <b>Modo Oscuro</b> desde el botón en la parte de abajo del menú lateral.',
          'El <b>Lector de Pantalla</b> (menú Acciones) lee en voz alta el botón o campo enfocado, para docentes o estudiantes con baja visión.',
        ],'')}
      `)}`,
    ...
    padre:`
      ...
      ${section('🆕 Funciones Recientes','#c0392b',`
        ${card('📢','Tablón de Anuncios','Se muestra automáticamente en su portal, arriba de "Mis Comunicados"',[
          'Vea los avisos que el docente de cada asignatura publique para el grado de su hijo/a: tareas, fechas de entrega, recordatorios.',
          'Los avisos más recientes aparecen primero. No necesita hacer nada para que quede registrado que lo consultó — el sistema lo marca automáticamente al abrir el Tablón.',
        ],'')}

        ${card('🔔','Activar Notificaciones Push','Portal de acudiente, botón "🔔 Activar" junto al Tablón de Anuncios',[
          'Al activarlas, recibirá una notificación en su celular aunque tenga la aplicación cerrada, cada vez que un docente publique un anuncio importante para el grado de su hijo/a.',
          'Debe aceptar el permiso que le pida el navegador para poder recibirlas.',
        ],'Funciona mejor si instaló la aplicación en su celular (opción "Instalar" o "Agregar a pantalla de inicio" del navegador).')}

        ${card('🌙','Modo Oscuro','Botón en la parte de abajo del menú, o en el encabezado del portal',[
          'Cambia los colores de toda la pantalla para reducir el cansancio visual, especialmente de noche.',
        ],'')}
      `)}`,
    ...
    estudiante:`
      ...
      ${section('🆕 Funciones Recientes','#c0392b',`
        ${card('📢','Tablón de Anuncios','Se muestra automáticamente en tu portal',[
          'Consulta los avisos que tus docentes publiquen para tu grado: tareas, fechas de entrega, recordatorios.',
          'Se marca solo que ya lo viste — no tienes que hacer nada aparte.',
        ],'')}

        ${card('🔔','Activar Notificaciones','Portal del estudiante, botón "🔔 Activar" junto al Tablón de Anuncios',[
          'Recibe una notificación en tu celular aunque tengas la aplicación cerrada, cuando un docente publique un anuncio para tu grado.',
        ],'')}

        ${card('🌙','Modo Oscuro','Botón en la parte de abajo del menú, o en el encabezado del portal',[
          'Cambia los colores de la pantalla para que sea más cómoda de ver, sobre todo de noche.',
        ],'')}
      `)}`,
  };
  ...
  <div class="card" style="padding:16px 20px">
    ...
    <div style="max-height:72vh;overflow-y:auto;padding-right:6px;scrollbar-width:thin">
      ${contenidos[activeTab]||contenidos[tabs[0]?.id]||_htmlEstadoVacio('🔎','Seleccione un rol para ver el manual.')}
    </div>
  </div>`;
}
function generarManualPDF(rol){
  ...
  const manuales={
    admin:{titulo:'MANUAL DE USUARIO — ADMINISTRADOR / RECTOR',secciones:[
      ...
      {t:'18. NOTAS DE ACTIVIDADES EN CLASE',c:'Registre notas de talleres, actividades en clase/casa y participación; el promedio se sincroniza automáticamente con la Planilla de Calificaciones.'},
      {t:'19. CALENDARIO ACADÉMICO',c:'Vea en un solo calendario sus eventos, los festivos colombianos calculados automáticamente y las fechas del Cronograma de Notas.'},
      {t:'20. COMPARATIVO ENTRE AÑOS',c:'Desde Histórico de Años, compare promedio y % de aprobación por grado entre el año actual y un año archivado.'},
      {t:'21. COORDINADORES DE SOLO LECTURA',c:'Delegue en un coordinador o auxiliar un acceso que ve todo el panel pero no puede guardar ni eliminar nada. Se gestiona desde Carga Académica.'},
      {t:'22. FIRMA DIGITAL Y VERIFICACIÓN DE DOCUMENTOS',c:'Los boletines y actas incluyen un código y un QR firmados por el servidor. Verifíquelos desde Documentos/Actas → Verificar Documento (texto o foto del QR).'},
      {t:'23. AUDITORÍA DE MATRÍCULA Y TRASLADOS',c:'Consulte quién trasladó a un estudiante de grado y cuándo, desde Historial de Cambios → Matrícula y Traslados. Exportable a PDF firmado.'},
      {t:'24. ATAJOS DE TECLADO EN PLANILLA',c:'Use las flechas para moverse entre celdas de notas y Enter/Espacio para abrir el selector, sin necesidad del mouse.'},
      {t:'25. MODO OFFLINE',c:'Si se corta la conexión, los cambios se guardan localmente y se envían solos al reconectar — no se pierden.'},
      {t:'26. LECTOR DE PANTALLA Y NAVEGACIÓN POR VOZ',c:'Desde el menú Acciones: el Lector de Pantalla lee en voz alta lo que recibe el foco; la Navegación por Voz permite hablar el nombre de un módulo para ir hacia allá.'},
      {t:'27. RECORRIDO GUIADO',c:'Desde el Tablero, un recorrido paso a paso resalta en el menú cada función principal del sistema.'},
      {t:'28. MODO OSCURO Y APLICACIÓN INSTALABLE',c:'Active el modo oscuro desde la barra lateral, e instale el sistema como aplicación desde la opción "Instalar" de su navegador.'},
    ]},
    docente:{titulo:'MANUAL DE USUARIO — DOCENTE',secciones:[
      ...
      {t:'11. TABLÓN DE ANUNCIOS',c:'Publique avisos para el grado (tareas, entregas, recordatorios). Cada aviso muestra el % de estudiantes que ya lo consultaron.'},
      {t:'12. NOTAS DE ACTIVIDADES EN CLASE',c:'Registre notas de talleres y participación; se sincronizan automáticamente con la Planilla.'},
      {t:'13. ATAJOS DE TECLADO EN PLANILLA',c:'Use las flechas para moverse entre celdas y Enter/Espacio para calificar sin el mouse.'},
      {t:'14. MODO OFFLINE',c:'Si pierde la conexión, sus notas se guardan localmente y se envían solas al reconectar.'},
      {t:'15. MODO OSCURO Y LECTOR DE PANTALLA',c:'Active el modo oscuro desde la barra lateral, y el lector de pantalla desde el menú Acciones si necesita apoyo visual.'},
    ]},
    padre:{titulo:'MANUAL DE USUARIO — PADRE DE FAMILIA / ACUDIENTE',secciones:[
      ...
      {t:'6. TABLÓN DE ANUNCIOS',c:'Consulte los avisos que los docentes publiquen para el grado de su hijo(a); queda registrado automáticamente que lo vio.'},
      {t:'7. NOTIFICACIONES PUSH',c:'Actívelas desde el botón junto al Tablón de Anuncios para recibir avisos en su celular aunque tenga la app cerrada.'},
      {t:'8. MODO OSCURO',c:'Actívelo desde el botón en el menú o el encabezado del portal.'},
    ]},
    estudiante:{titulo:'MANUAL DE USUARIO — ESTUDIANTE',secciones:[
      ...
      {t:'6. TABLÓN DE ANUNCIOS',c:'Consulta los avisos que tus docentes publiquen para tu grado.'},
      {t:'7. NOTIFICACIONES',c:'Actívalas para recibir avisos en tu celular aunque tengas la app cerrada.'},
      {t:'8. MODO OSCURO',c:'Actívalo desde el botón en el menú o el encabezado del portal.'},
    ]}
  };
  ...
  doc.setFontSize(13);doc.text(instNombre,105,75,{align:'center',maxWidth:170});
  doc.setFillColor(241,196,15);doc.rect(30,90,150,1,'F');
  doc.setFontSize(16);doc.text(manual.titulo,105,110,{align:'center',maxWidth:170});
...
async function responderPermiso(id,decision){
  ...
  const motivoRec=decision==='Rechazado'?(await customPrompt('Motivo del rechazo (opcional):','')):'';
  updDB(d=>{
    const idx=(d.ausentismos||[]).findIndex(s=>s.id===id);
    if(idx>=0){
  ...
  try{fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual(),kind:'respuesta-permiso',actor:db.rectora||'Rector(a)',message:msg,meta:{docente:docNombre,decision,fecha:new Date().toISOString()}})}).catch(()=>{});}catch(e){}
...
function htmlSeguimientoObservador(){
  ...
  const ests=db.ests||[];
  let todasObsIni=[];
  ests.forEach(e=>{(e.observaciones||[]).forEach(o=>{todasObsIni.push({doc:o.doc||''});});});
  const docsUnicos=[...new Set(todasObsIni.map(o=>o.doc).filter(Boolean))].sort();
  ...
  <div class="card" style="margin-bottom:14px">
    <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
      <div>
        ...
        <select onchange="window._soGrado=this.value;_refrescarSegObsResultados()" style="padding:6px 10px;border:1px solid #c0cfe0;border-radius:6px;font-size:0.85rem">
          <option value="">Todos los grados</option>
      ...
      <div>
        ...
        <select onchange="window._soPer=this.value;_refrescarSegObsResultados()" style="padding:6px 10px;border:1px solid #c0cfe0;border-radius:6px;font-size:0.85rem">
      ...
      <div>
        ...
        <select onchange="window._soDoc=this.value;_refrescarSegObsResultados()" style="padding:6px 10px;border:1px solid #c0cfe0;border-radius:6px;font-size:0.85rem">
      ...
      <div>
        ...
        <input value="${window._soBusq||''}" placeholder="Nombre o palabra..." oninput="_onBuscarSegObs(this.value)" style="padding:6px 10px;border:1px solid #c0cfe0;border-radius:6px;font-size:0.85rem;min-width:180px">
      </div>
  ...
  <div id="segObsResultados">${_htmlSegObsResultados()}</div>`;
}

// Recalcula solo el bloque de resultados (tarjetas, gráfica, tabla y últimas
// observaciones) sin reconstruir toda la página — así el campo de búsqueda
// nunca pierde el foco ni la posición del scroll mientras se escribe.
let _segObsBuscarTimer=null;
function _onBuscarSegObs(v){
  window._soBusq=v;
  if(_segObsBuscarTimer) clearTimeout(_segObsBuscarTimer);
  _segObsBuscarTimer=setTimeout(_refrescarSegObsResultados,220);
}
function _refrescarSegObsResultados(){
  const wrap=document.getElementById('segObsResultados');
  if(wrap) wrap.innerHTML=_htmlSegObsResultados();
}
function _htmlSegObsResultados(){
  const ests=db.ests||[];
  const filGrado=window._soGrado||'';
  const filPer=window._soPer||'0';
  const filDoc=window._soDoc||'';
  ...
  const listaCompromisos=ultimas.length?ultimas.map(o=>`<div style="border-left:3px solid #1a5276;padding:8px 12px;margin-bottom:8px;background:#f9fbff;border-radius:0 6px 6px 0">
    ...
    <p style="margin:4px 0 0;font-size:0.83rem;color:var(--text-secondary)">${o.txt}</p>
  ...
  const timeline=obs.length?obs.map((o,i)=>{
    ...
    return `<div style="display:flex;gap:12px;margin-bottom:12px;align-items:flex-start">
      ...
      <div style="flex:1;border:1px solid #e3eaf4;border-radius:8px;padding:10px 12px;background:${i%2===0?'#f7faff':'#fff'}">
        ...
        <p style="margin:0;font-size:0.87rem;color:var(--text-secondary);line-height:1.5">${o.txt||'(sin texto)'}</p>
  ...
  <div style="background:#fff;border-radius:14px;max-width:700px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 12px 48px rgba(0,0,0,.35)">
    ...
    <div style="padding:18px 20px">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;border-top:1px solid #e3eaf4;padding-top:14px">
        ...
        <button onclick="document.getElementById('${modalId}').remove()" style="background:#f0f4f8;color:#555;border:1px solid #c0cfe0;border-radius:8px;padding:9px 18px;cursor:pointer;font-size:0.85rem">Cerrar</button>
      </div>
    </div>
  </div>
</div>`;
...
// ============================================================
// CALENDARIO ACADÉMICO — vista unificada de "todo lo que viene":
// eventos propios (reuniones, entregas, avisos), festivos
// colombianos (calculados automáticamente, Ley Emiliani) y las
// fechas de apertura/cierre de periodo ya configuradas en el
// Cronograma de Notas. No reemplaza esos módulos — los reúne en
// una sola vista mensual para que el rector/docente vea de un
// vistazo qué se viene.
// ============================================================
function _pascuaFecha(anio){
...
function _sumarDiasFecha(fecha,dias){ const f=new Date(fecha); f.setDate(f.getDate()+dias); return f; }
function _lunesSiguienteFecha(fecha){
  const f=new Date(fecha);
  if(f.getDay()===1) return f;
  const dh=(8-f.getDay())%7;
  return _sumarDiasFecha(f,dh===0?7:dh);
}
function _festivosColombia(anio){
  ...
  const fijos=[[0,1,'Año Nuevo'],[4,1,'Día del Trabajo'],[6,20,'Día de la Independencia'],[7,7,'Batalla de Boyacá'],[11,8,'Inmaculada Concepción'],[11,25,'Navidad']];
  const trasladables=[[0,6,'Día de los Reyes Magos'],[2,19,'Día de San José'],[5,29,'San Pedro y San Pablo'],[7,15,'Asunción de la Virgen'],[9,12,'Día de la Raza'],[10,1,'Día de Todos los Santos'],[10,11,'Independencia de Cartagena']];
  const pascua=_pascuaFecha(anio);
  const movibles=[
    {fecha:_sumarDiasFecha(pascua,-3),nombre:'Jueves Santo'},
    {fecha:_sumarDiasFecha(pascua,-2),nombre:'Viernes Santo'},
    {fecha:_lunesSiguienteFecha(_sumarDiasFecha(pascua,39)),nombre:'Ascensión del Señor'},
    {fecha:_lunesSiguienteFecha(_sumarDiasFecha(pascua,60)),nombre:'Corpus Christi'},
    {fecha:_lunesSiguienteFecha(_sumarDiasFecha(pascua,68)),nombre:'Sagrado Corazón de Jesús'},
  ];
  const out=[];
  fijos.forEach(function(x){ out.push({fecha:fmt(new Date(anio,x[0],x[1])),nombre:x[2]}); });
  trasladables.forEach(function(x){ out.push({fecha:fmt(_lunesSiguienteFecha(new Date(anio,x[0],x[1]))),nombre:x[2]}); });
  movibles.forEach(function(x){ out.push({fecha:fmt(x.fecha),nombre:x.nombre}); });
  return out.sort(function(a,b){ return a.fecha.localeCompare(b.fecha); });
}
let _festivosCacheColombia={};
function _festivosColombiaCache(anio){
  if(!_festivosCacheColombia[anio]) _festivosCacheColombia[anio]=_festivosColombia(anio);
  return _festivosCacheColombia[anio];
}
const TIPO_EVENTO_INFO={
  evento:{label:'📌 Evento',color:'#2980b9'},
  entrega:{label:'📥 Entrega',color:'#8e44ad'},
  reunion:{label:'🤝 Reunión',color:'#e67e22'},
  festivo:{label:'🎉 Festivo',color:'#c0392b'},
  cronograma:{label:'📊 Notas',color:'#1e8449'},
};
function _todosLosEventos(anio){
  const eventos=[];
  (db.eventosCalendario||[]).forEach(function(ev){
    if(ev.deletedAt) return;
    eventos.push({fecha:ev.fecha,titulo:ev.titulo,tipo:ev.tipo,id:ev.id,grado:ev.grado||'',descripcion:ev.descripcion||'',creadoPor:ev.creadoPor||'',editable:true});
  });
  _festivosColombiaCache(anio).forEach(function(f){
    eventos.push({fecha:f.fecha,titulo:f.nombre,tipo:'festivo',editable:false});
  });
  const cr=db.cronograma||{};
  Object.keys(cr).forEach(function(k){
    const per=k.replace('p','');
    if(cr[k].desde) eventos.push({fecha:cr[k].desde,titulo:'Apertura notas — Periodo '+per,tipo:'cronograma',editable:false});
    if(cr[k].hasta) eventos.push({fecha:cr[k].hasta,titulo:'Cierre notas — Periodo '+per,tipo:'cronograma',editable:false});
  });
  return eventos;
}
// Escapa texto segun el estandar iCalendar (RFC 5545): la barra invertida,
// el punto y coma, la coma y los saltos de linea deben escaparse dentro de
// cualquier valor de tipo TEXT (SUMMARY, DESCRIPTION, etc.).
function _icsEscape(txt){
  return String(txt||'').replace(/\\/g,'\\\\').replace(/;/g,'\\;').replace(/,/g,'\\,').replace(/\n/g,'\\n');
}
// Exporta el Calendario Académico completo (eventos propios, festivos
// colombianos y fechas del Cronograma de Notas) como un archivo .ics
// estándar — se puede importar en Google Calendar, Outlook, Apple
// Calendar o cualquier aplicación de calendario que soporte el formato
// iCalendar (RFC 5545), para que cada docente lo tenga sincronizado en
// su propio calendario personal.
function _exportarCalendarioICS(){
  const anioActual=new Date().getFullYear();
  // Se incluyen 2 años (el actual y el siguiente) para cubrir eventos que
  // ya estén programados hacia adelante, sin generar un archivo enorme
  // con años que no aplican.
  const eventos=_todosLosEventos(anioActual).concat(_todosLosEventos(anioActual+1));
  if(!eventos.length){ customAlert('No hay eventos para exportar todavía.'); return; }
  const ahoraStamp=new Date().toISOString().replace(/[-:]/g,'').split('.')[0]+'Z';
  const lineas=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Gestor Académico YC//Calendario Académico//ES','CALSCALE:GREGORIAN','METHOD:PUBLISH'];
  const vistos=new Set(); // evitar eventos duplicados (ej. si un festivo aparece en ambos años consultados)
  eventos.forEach(function(ev,i){
    const fechaCompacta=ev.fecha.replace(/-/g,'');
    const clave=fechaCompacta+'|'+ev.titulo;
    if(vistos.has(clave)) return;
    vistos.add(clave);
    lineas.push('BEGIN:VEVENT');
    lineas.push('UID:gestoryc-'+fechaCompacta+'-'+i+'@gestoracademicoyc');
    lineas.push('DTSTAMP:'+ahoraStamp);
    lineas.push('DTSTART;VALUE=DATE:'+fechaCompacta);
    lineas.push('SUMMARY:'+_icsEscape(ev.titulo));
    if(ev.descripcion) lineas.push('DESCRIPTION:'+_icsEscape(ev.descripcion+(ev.grado?' (Grado: '+ev.grado+')':'')));
    else if(ev.grado) lineas.push('DESCRIPTION:'+_icsEscape('Grado: '+ev.grado));
    lineas.push('CATEGORIES:'+_icsEscape((TIPO_EVENTO_INFO[ev.tipo]&&TIPO_EVENTO_INFO[ev.tipo].label)||ev.tipo));
    lineas.push('END:VEVENT');
  });
  lineas.push('END:VCALENDAR');
  // Los saltos de línea del formato iCalendar deben ser CRLF según el
  // estándar (la mayoría de apps son flexibles, pero se respeta para
  ...
  const a=document.createElement('a');
  a.href=url;
  a.download='Calendario_Academico_'+(db.anio||anioActual)+'.ics';
  document.body.appendChild(a);a.click();document.body.removeChild(a);
  URL.revokeObjectURL(url);
  _showToast('📆 Calendario exportado — ábralo con Google Calendar, Outlook o su app de calendario para importarlo.','success',5000);
}
function htmlCalendarioAcademico(){
  if(window._calAnio===undefined){ window._calAnio=new Date().getFullYear(); window._calMes=new Date().getMonth(); }
  const anio=window._calAnio, mes=window._calMes;
  const nombreMes=new Date(anio,mes,1).toLocaleDateString('es-CO',{month:'long',year:'numeric'});
  const todos=_todosLosEventos(anio).concat(mes===11?_todosLosEventos(anio+1):[]).concat(mes===0?_todosLosEventos(anio-1):[]);
  const eventosMes=todos.filter(function(ev){ const d=new Date(ev.fecha+'T00:00:00'); return d.getFullYear()===anio&&d.getMonth()===mes; });
  const eventosPorDia={};
  eventosMes.forEach(function(ev){
    const dia=parseInt(ev.fecha.slice(8,10),10);
    if(!eventosPorDia[dia]) eventosPorDia[dia]=[];
    eventosPorDia[dia].push(ev);
  });
  const primerDia=new Date(anio,mes,1),diasEnMes=new Date(anio,mes+1,0).getDate(),diaSemanaInicio=primerDia.getDay();
  const hoy=new Date(),esMesActual=hoy.getFullYear()===anio&&hoy.getMonth()===mes;
  let celdas='<tr>';
  for(let i=0;i<diaSemanaInicio;i++) celdas+='<td class="cal-celda-vacia"></td>';
  for(let dia=1;dia<=diasEnMes;dia++){
    ...
    celdas+='<td style="vertical-align:top;padding:4px;border:1px solid var(--border-color);'+(esHoy?'background:#fff8e1':'')+'">'
      ...
      +'</td>';
    if((dia+diaSemanaInicio)%7===0) celdas+='</tr><tr>';
  }
  celdas+='</tr>';
  const proximos=todos.filter(function(ev){ return ev.fecha>=hoy.toISOString().slice(0,10); }).sort(function(a,b){ return a.fecha.localeCompare(b.fecha); }).slice(0,15);
  const isAdmin=sesion.r==='admin';
  const misGrados=isAdmin?(db.grados||[]).map(function(g){return g.n;}):[...new Set((db.carga||[]).filter(function(c){return c.d===sesion.u;}).map(function(c){return c.g;}))];
  return `<h3 class="sec-title">📅 Calendario Académico</h3>
  <div class="info-box" style="margin-bottom:14px">ℹ️ Reúne en un solo lugar sus eventos, los festivos colombianos y las fechas del Cronograma de Notas. No reemplaza esos módulos, solo los muestra juntos.</div>
  <div class="card" style="margin-bottom:16px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px">
      <div style="display:flex;align-items:center;gap:10px">
        <button class="btn-sm" style="background:#7f8c8d" onclick="_calNavegarMes(-1)">◀</button>
        <h4 style="margin:0;text-transform:capitalize;color:#003366">${nombreMes}</h4>
        <button class="btn-sm" style="background:#7f8c8d" onclick="_calNavegarMes(1)">▶</button>
        <button class="btn-sm" style="background:#2980b9" onclick="window._calAnio=new Date().getFullYear();window._calMes=new Date().getMonth();renderApp()">Hoy</button>
        <button class="btn-sm" style="background:#16a085" onclick="_exportarCalendarioICS()" title="Descargar como archivo .ics para importar en Google Calendar, Outlook, Apple Calendar, etc.">📆 Exportar a Calendario</button>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;font-size:0.7rem">
        ${Object.values(TIPO_EVENTO_INFO).map(function(t){ return '<span><span style="display:inline-block;width:9px;height:9px;background:'+t.color+';border-radius:2px;margin-right:3px"></span>'+t.label+'</span>'; }).join('')}
      </div>
    </div>
    <div class="over"><table style="width:100%;border-collapse:collapse;table-layout:fixed">
      <thead><tr>${['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'].map(function(d){return '<th style="padding:5px;font-size:0.75rem;color:#888">'+d+'</th>';}).join('')}</tr></thead>
      <tbody>${celdas}</tbody>
    </table></div>
  </div>
  <div style="display:grid;grid-template-columns:1.3fr 1fr;gap:16px" class="cal-grid-resp">
    <div class="card">
      <h4 class="card-title">🔜 Próximos Eventos</h4>
      ${proximos.length?proximos.map(function(ev){
        return '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:8px 10px;border-left:4px solid '+TIPO_EVENTO_INFO[ev.tipo].color+';background:var(--bg-card-hover);border-radius:0 6px 6px 0;margin-bottom:6px">'
        +'<div><b style="font-size:0.85rem">'+ev.titulo+'</b><br><span style="font-size:0.74rem;color:#888">'+new Date(ev.fecha+'T00:00:00').toLocaleDateString('es-CO',{weekday:'short',day:'numeric',month:'short'})+(ev.grado?' · '+ev.grado:'')+'</span></div>'
        +(ev.editable&&(isAdmin||ev.creadoPor===sesion.u)?'<button class="btn-sm" style="background:#c0392b" onclick="_eliminarEventoCalendario('+ev.id+')">🗑</button>':'')
        +'</div>';
      }).join(''):_htmlEstadoVacio('📅','No hay eventos próximos registrados.')}
    </div>
    <div class="card">
      <h4 class="card-title">+ Agregar Evento</h4>
      <div style="margin-bottom:8px"><label class="lbl">Título *</label><input id="calTitulo" placeholder="Ej: Entrega de boletines"></div>
      <div style="margin-bottom:8px"><label class="lbl">Fecha *</label><input type="date" id="calFecha"></div>
      <div style="margin-bottom:8px"><label class="lbl">Tipo</label><select id="calTipo">
        <option value="evento">📌 Evento</option>
        <option value="entrega">📥 Entrega</option>
        <option value="reunion">🤝 Reunión</option>
      </select></div>
      <div style="margin-bottom:8px"><label class="lbl">Grado (opcional)</label><select id="calGrado"><option value="">Todos / General</option>${misGrados.map(function(g){return '<option value="'+g+'">'+g+'</option>';}).join('')}</select></div>
      <div style="margin-bottom:10px"><label class="lbl">Descripción (opcional)</label><input id="calDesc" placeholder="Detalles adicionales"></div>
      <button class="btn btn-green" onclick="_agregarEventoCalendario()">+ Agregar</button>
    </div>
  </div>`;
}
function _calNavegarMes(delta){
  let m=window._calMes+delta, a=window._calAnio;
  if(m<0){m=11;a--;} else if(m>11){m=0;a++;}
  window._calMes=m; window._calAnio=a;
  renderApp();
}
function _agregarEventoCalendario(){
  const titulo=document.getElementById('calTitulo')?.value.trim();
  const fecha=document.getElementById('calFecha')?.value;
  const tipo=document.getElementById('calTipo')?.value||'evento';
  const grado=document.getElementById('calGrado')?.value||'';
  const descripcion=document.getElementById('calDesc')?.value.trim()||'';
  if(!titulo||!fecha){ customAlert('Complete al menos el título y la fecha.'); return; }
  updDB(function(d){
    d.eventosCalendario=d.eventosCalendario||[];
    const nuevoId=(d.eventosCalendario.reduce(function(m,e){return Math.max(m,e.id||0);},0))+1;
    d.eventosCalendario.push({id:nuevoId,titulo,fecha,tipo,grado,descripcion,creadoPor:sesion.u});
    return d;
  });
  renderApp();
}
async function _eliminarEventoCalendario(id){
  if(!await customConfirm('¿Eliminar este evento del calendario?')) return;
  updDB(function(d){ d.eventosCalendario=(d.eventosCalendario||[]).filter(function(e){return e.id!==id;}); return d; });
  renderApp();
}

// ============================================================
// PANEL DE TENDENCIAS — evolución del desempeño institucional a lo
// largo del tiempo, no solo el dato del momento actual.
// ------------------------------------------------------------------
// LIMITACIÓN HONESTA: los años archivados (ver "Histórico de Años")
// solo guardan notas de los estudiantes, NO los registros diarios de
// asistencia — por eso la tendencia de asistencia se muestra POR
// PERIODO dentro del año actual (donde sí hay datos), y la tendencia
// de promedio se muestra ENTRE AÑOS (donde sí se archiva la
// información necesaria). Si en el futuro se empieza a archivar
// también la asistencia por año, se podría ampliar esta segunda
// gráfica a varios años igual que la primera.
//
// El promedio institucional que se calcula aquí es una cifra
// SIMPLIFICADA con fines de tendencia (promedio simple de los
// componentes Ser/Saber/Hacer de cada nota registrada) — no aplica
// los pesos/porcentajes configurables que sí usa el boletín oficial,
// ya que esos pesos pueden haber cambiado de un año a otro y no
// quedan archivados de forma que se puedan reconstruir con certeza.
// Sirve para ver la TENDENCIA (¿va subiendo o bajando?), no como
// reemplazo del promedio oficial de cada estudiante.
// ============================================================
function _promedioSimpleDeEstudiantes(ests){
  let suma=0,cuenta=0;
  (ests||[]).forEach(function(e){
    const nts=e.nts||{};
    Object.keys(nts).forEach(function(materiaId){
      Object.keys(nts[materiaId]||{}).forEach(function(periodo){
        const n=nts[materiaId][periodo];
        if(!n) return;
        const vals=[n.s,n.sb,n.h].filter(function(v){return typeof v==='number'&&v>0;});
        if(vals.length){ suma+=vals.reduce(function(a,b){return a+b;},0)/vals.length; cuenta++; }
      });
    });
  });
  return cuenta>0?suma/cuenta:null;
}
function _datosTendenciaAnios(){
  const hist=(db.historialAnios||[]).slice().sort(function(a,b){return String(a.anio).localeCompare(String(b.anio));});
  const puntos=hist.map(function(h){
    return {anio:h.anio,promedio:_promedioSimpleDeEstudiantes(h.datos&&h.datos.ests)};
  });
  // Agregar el año actual (en vivo, con los datos de ahora mismo)
  puntos.push({anio:db.anio,promedio:_promedioSimpleDeEstudiantes(db.ests),actual:true});
  return puntos.filter(function(p){return p.promedio!==null;});
}
function _datosTendenciaAsistenciaPorPeriodo(){
  const numPer=(db.config&&db.config.numPeriodos)||4;
  const puntos=[];
  for(let p=1;p<=numPer;p++){
    const clases=(db.asistencia||[]).filter(function(a){return !a.deletedAt&&String(a.periodo)==='P'+p;});
    if(!clases.length){ puntos.push({periodo:'P'+p,pctAsistencia:null}); continue; }
    let totalPosible=0,totalAusentes=0;
    clases.forEach(function(c){
      const numEstClase=(c.ausentes||[]).length+(c.justificados||[]).length+_estimarPresentesClase(c);
      totalPosible+=numEstClase;
      totalAusentes+=(c.ausentes||[]).length;
    });
    puntos.push({periodo:'P'+p,pctAsistencia:totalPosible>0?100-(totalAusentes/totalPosible*100):null});
  }
  return puntos;
}
// Aproximación: como no se guarda cuántos estudiantes tenía el grado en el
// momento exacto de cada clase histórica, se usa el conteo actual de
// estudiantes de ese grado como estimado razonable (asume que el grado no
// cambió drásticamente de tamaño durante el periodo).
function _estimarPresentesClase(registroClase){
  const totalGrado=(db.ests||[]).filter(function(e){return !e.deletedAt&&e.g===registroClase.grado;}).length;
  const marcados=(registroClase.ausentes||[]).length+(registroClase.justificados||[]).length;
  return Math.max(0,totalGrado-marcados);
}
function htmlPanelTendencias(){
  const datosAnios=_datosTendenciaAnios();
  const datosAsist=_datosTendenciaAsistenciaPorPeriodo();
  const chartIdProm='chartTendProm_'+Date.now();
  const chartIdAsist='chartTendAsist_'+Date.now();
  setTimeout(function(){
    try{
      if(datosAnios.length>=1){
        const ctx=document.getElementById(chartIdProm);
        if(ctx) new Chart(ctx,{type:'line',data:{labels:datosAnios.map(function(p){return String(p.anio)+(p.actual?' (actual)':'');}),
          datasets:[{label:'Promedio institucional (indicador de tendencia)',data:datosAnios.map(function(p){return p.promedio.toFixed(2);}),borderColor:'#003366',backgroundColor:'rgba(0,51,102,0.12)',fill:true,tension:0.25,pointRadius:5}]},
          options:{responsive:true,scales:{y:{min:0,max:5}},plugins:{legend:{display:false}}}});
      }
      const conDatosAsist=datosAsist.filter(function(p){return p.pctAsistencia!==null;});
      if(conDatosAsist.length>=1){
        const ctx2=document.getElementById(chartIdAsist);
        if(ctx2) new Chart(ctx2,{type:'bar',data:{labels:datosAsist.map(function(p){return p.periodo;}),
          datasets:[{label:'% Asistencia',data:datosAsist.map(function(p){return p.pctAsistencia!==null?p.pctAsistencia.toFixed(1):0;}),backgroundColor:'#16a085'}]},
          options:{responsive:true,scales:{y:{min:0,max:100}},plugins:{legend:{display:false}}}});
      }
    }catch(e){}
  },80);
  return `<h3 class="sec-title">📈 Panel de Tendencias</h3>
  <div class="info-box" style="margin-bottom:14px">ℹ️ Muestra cómo ha evolucionado el desempeño institucional a lo largo del tiempo, en vez de solo el dato del momento actual. El promedio entre años es un indicador simplificado de tendencia (no reemplaza el promedio oficial de cada estudiante). La asistencia se muestra por periodo del año actual, ya que los años archivados no guardan el detalle diario de asistencia.</div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" class="cal-grid-resp">
    <div class="card">
      <h4 class="card-title">📊 Promedio Institucional — Evolución entre Años</h4>
      ${datosAnios.length?`<div style="max-height:280px"><canvas id="${chartIdProm}"></canvas></div>`:_htmlEstadoVacio('📈','Todavía no hay años archivados para comparar. Use "Histórico de Años" para archivar el cierre de cada año lectivo.')}
    </div>
    <div class="card">
      <h4 class="card-title">🗓️ Asistencia — Evolución por Periodo (año ${db.anio})</h4>
      ${datosAsist.some(function(p){return p.pctAsistencia!==null;})?`<div style="max-height:280px"><canvas id="${chartIdAsist}"></canvas></div>`:_htmlEstadoVacio('🗓️','Todavía no hay registros de asistencia este año.')}
    </div>
  </div>`;
}
...
// y % de aprobación por grado entre el año actual y un año
// archivado en el histórico, usando exactamente el mismo método de
// cálculo (promedio ponderado por área) que ya usa el Tablero, para
// que las cifras sean consistentes entre vistas.
// ============================================================
function _statsAnioDesdeDatos(datosAnio){
  const dbReal=db;
  try{
    db={ests:datosAnio.ests||[],carga:datosAnio.carga||[],grados:datosAnio.grados||[],config:datosAnio.config||dbReal.config||{}};
    return (db.grados||[]).map(function(g){
      const estsG=db.ests.filter(function(e){return e.g===g.n;});
      const matsG=db.carga.filter(function(c){return c.g===g.n;});
      if(!matsG.length||!estsG.length) return {grado:g.n,prom:0,pctApr:0,pctRep:0,total:estsG.length,conNotas:0};
      let ap=0,re=0,sp=0,cn=0;
      estsG.forEach(function(e){
        const hasMat=matsG.some(function(m){ for(let p=1;p<=_getNumPer();p++){ if(calcNotaDef(e.nts,m.id,p)>0) return true; } return false; });
        if(!hasMat) return;
        cn++;const pr=calcPromedioEst(e.id,g.n);sp+=pr;
        if(pr>=3.0) ap++; else re++;
      });
      return {grado:g.n,prom:cn?parseFloat((sp/cn).toFixed(2)):0,pctApr:cn?Math.round(ap/cn*100):0,pctRep:cn?Math.round(re/cn*100):0,total:estsG.length,conNotas:cn};
    });
  }catch(e){
    return [];
  }finally{
...
function _htmlComparativoAnios(hist,anioActivo){
  const aniosConDatos=[anioActivo].concat((hist||[]).filter(function(h){return h.datos;}).map(function(h){return h.anio;}));
  const aniosUnicos=[...new Set(aniosConDatos)];
  if(aniosUnicos.length<2){
    return `<div class="card" style="margin-top:16px;border-left:4px solid #8e44ad">
      <h4 class="card-title" style="color:#8e44ad">📈 Comparativo entre Años</h4>
      <p style="font-size:0.85rem;color:#666">Archive al menos un año con "📦 Archivar Año Actual" para poder comparar promedios y aprobación entre años.</p>
    </div>`;
  }
  const anioA=window._compAnioA&&aniosUnicos.includes(window._compAnioA)?window._compAnioA:aniosUnicos[0];
  const anioB=window._compAnioB&&aniosUnicos.includes(window._compAnioB)?window._compAnioB:aniosUnicos[aniosUnicos.length-1];
  const opts=function(sel){ return aniosUnicos.map(function(a){ return '<option value="'+a+'"'+(a===sel?' selected':'')+'>'+a+(a===anioActivo?' (actual)':'')+'</option>'; }).join(''); };
  return `<div class="card" style="margin-top:16px;border-left:4px solid #8e44ad" id="comparativoAniosCard">
    <h4 class="card-title" style="color:#8e44ad">📈 Comparativo entre Años</h4>
    <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:14px">
      <div><label class="lbl">Año A</label><select id="compAnioASel" onchange="window._compAnioA=this.value;_refrescarComparativoAnios()">${opts(anioA)}</select></div>
      <div><label class="lbl">Año B</label><select id="compAnioBSel" onchange="window._compAnioB=this.value;_refrescarComparativoAnios()">${opts(anioB)}</select></div>
      <button class="btn btn-navy" onclick="pdfComparativoAnios()">🖨️ Exportar PDF</button>
    </div>
    <div id="comparativoAniosResultado">${_htmlComparativoAniosResultado(anioA,anioB,hist,anioActivo)}</div>
  </div>`;
}
function _datosDelAnio(anio,hist,anioActivo){
  if(anio===anioActivo) return {ests:db.ests,carga:db.carga,grados:db.grados,config:db.config};
  const h=(hist||[]).find(function(x){return x.anio===anio;});
  return h?h.datos:null;
}
function _refrescarComparativoAnios(){
  const anioA=document.getElementById('compAnioASel')?.value||window._compAnioA;
  const anioB=document.getElementById('compAnioBSel')?.value||window._compAnioB;
  const hist=db.historialAnios||[];
  const platId=gestorEnPlataforma||window._currentPlatId;
  const plat=platId?gestorDB.platforms.find(function(x){return x.id===platId;}):null;
  const anioActivo=plat?plat.anioActivo:db.anio;
  const cont=document.getElementById('comparativoAniosResultado');
  if(cont) cont.innerHTML=_htmlComparativoAniosResultado(anioA,anioB,hist,anioActivo);
}
function _htmlComparativoAniosResultado(anioA,anioB,hist,anioActivo){
  const datosA=_datosDelAnio(anioA,hist,anioActivo);
  const datosB=_datosDelAnio(anioB,hist,anioActivo);
  if(!datosA||!datosB) return _htmlEstadoVacio('🔎','No hay datos completos para uno de los años seleccionados.');
  const statsA=_statsAnioDesdeDatos(datosA);
  const statsB=_statsAnioDesdeDatos(datosB);
  const gradosTodos=[...new Set([...statsA.map(function(s){return s.grado;}),...statsB.map(function(s){return s.grado;})])].sort();
  if(!gradosTodos.length) return _htmlEstadoVacio('📝','Ninguno de los dos años tiene grados con notas registradas para comparar.');
  const filas=gradosTodos.map(function(g){
    const a=statsA.find(function(s){return s.grado===g;})||{prom:0,pctApr:0};
    const b=statsB.find(function(s){return s.grado===g;})||{prom:0,pctApr:0};
    const deltaProm=parseFloat((a.prom-b.prom).toFixed(2));
    const deltaApr=a.pctApr-b.pctApr;
    return `<tr>
      <td style="text-align:left"><b>${g}</b></td>
      <td>${b.prom.toFixed(2)}</td><td>${a.prom.toFixed(2)}</td>
      <td style="color:${deltaProm>=0?'#1e8449':'#c0392b'};font-weight:bold">${deltaProm>=0?'▲':'▼'} ${Math.abs(deltaProm).toFixed(2)}</td>
      <td>${b.pctApr}%</td><td>${a.pctApr}%</td>
      <td style="color:${deltaApr>=0?'#1e8449':'#c0392b'};font-weight:bold">${deltaApr>=0?'▲':'▼'} ${Math.abs(deltaApr)}%</td>
    </tr>`;
  }).join('');
  const chartId='chCompAnios_'+Date.now();
  setTimeout(function(){
    if(typeof Chart==='undefined') return;
    const ctx=document.getElementById(chartId);
    if(!ctx) return;
    new Chart(ctx,{type:'bar',data:{labels:gradosTodos,datasets:[
      {label:'Promedio '+anioB,data:gradosTodos.map(function(g){const s=statsB.find(function(x){return x.grado===g;});return s?s.prom:0;}),backgroundColor:'#95a5a6'},
      {label:'Promedio '+anioA,data:gradosTodos.map(function(g){const s=statsA.find(function(x){return x.grado===g;});return s?s.prom:0;}),backgroundColor:'#8e44ad'},
    ]},options:{responsive:true,scales:{y:{beginAtZero:true,max:5}},plugins:{title:{display:true,text:'Promedio por grado: '+anioB+' vs '+anioA}}}});
  },200);
  return `<div style="max-height:320px;margin-bottom:16px"><canvas id="${chartId}"></canvas></div>
  <div class="over"><table><thead><tr style="background:#8e44ad;color:#fff">
    <th style="text-align:left">Grado</th><th>Prom. ${anioB}</th><th>Prom. ${anioA}</th><th>Δ Promedio</th>
    <th>Aprob. ${anioB}</th><th>Aprob. ${anioA}</th><th>Δ Aprobación</th>
  </tr></thead><tbody>${filas}</tbody></table></div>`;
}
function pdfComparativoAnios(){
  const anioA=window._compAnioA,anioB=window._compAnioB;
  if(!anioA||!anioB){customAlert('Seleccione ambos años primero.');return;}
  const hist=db.historialAnios||[];
  const platId=gestorEnPlataforma||window._currentPlatId;
  const plat=platId?gestorDB.platforms.find(function(x){return x.id===platId;}):null;
  const anioActivo=plat?plat.anioActivo:db.anio;
  const datosA=_datosDelAnio(anioA,hist,anioActivo), datosB=_datosDelAnio(anioB,hist,anioActivo);
  if(!datosA||!datosB){customAlert('Faltan datos de alguno de los años.');return;}
  const statsA=_statsAnioDesdeDatos(datosA), statsB=_statsAnioDesdeDatos(datosB);
  const gradosTodos=[...new Set([...statsA.map(function(s){return s.grado;}),...statsB.map(function(s){return s.grado;})])].sort();
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF('p','mm','a4');
  doc.setFontSize(14);doc.text('Comparativo Institucional: '+anioB+' vs '+anioA,14,18);
  doc.setFontSize(10);doc.text(db.nombre||'Institución',14,25);
  const filas=gradosTodos.map(function(g){
    const a=statsA.find(function(s){return s.grado===g;})||{prom:0,pctApr:0};
    const b=statsB.find(function(s){return s.grado===g;})||{prom:0,pctApr:0};
    return [g,b.prom.toFixed(2),a.prom.toFixed(2),(a.prom-b.prom).toFixed(2),b.pctApr+'%',a.pctApr+'%',(a.pctApr-b.pctApr)+'%'];
  });
  doc.autoTable({startY:32,head:[['Grado','Prom. '+anioB,'Prom. '+anioA,'Δ Prom.','Aprob. '+anioB,'Aprob. '+anioA,'Δ Aprob.']],body:filas,styles:{fontSize:8},headStyles:{fillColor:[142,68,173]}});
  doc.save('Comparativo_'+anioB+'_vs_'+anioA+'.pdf');
}
...
async function loginBiometricoPortal(platId){
  ...
  try{
    ...
    if(user.r==='docente'){
      try{fetch(API_BASE+'/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({sk:_skActual(),kind:'login',actor:user.n,message:'El/La docente '+user.n+' ingresó al sistema (biométrico)',meta:{u:user.u,fecha:new Date().toISOString()}})}).catch(()=>{});}catch(e){}
    }
    _sseInit();
...
async function enviarRecuperacionGestor(){
  ...
  // Por seguridad, la contraseña ya no se guarda en texto plano: se genera una temporal nueva,
  // se guarda ya cifrada, y esa es la que se envía por correo.
  const password=_generarPasswordTemporal();
...
async function _enviarCorreoRecuperacion(destinatario,nombreUsuario,usuario,password,rol,nombrePlat){
  ...
  try{
    const resp=await fetch('/api/inetis/send-email',{
...
function _generarPasswordTemporal(){
  ...
  let out='';
  const rnd=new Uint32Array(10);crypto.getRandomValues(rnd);
  ...
  for(const plat of gestorDB.platforms){
    ...
    try{
      ...
      if(user){
        ...
        try{
          await fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},
            body:JSON.stringify({sk:_skActual(),kind:'recuperacion-password',actor:user.n||user.u,
        ...
        // Por seguridad, la contraseña ya no se guarda en texto plano y no puede leerse de vuelta:
        // se genera una contraseña temporal nueva, se guarda ya cifrada, y esa es la que se envía por correo.
        const passTemporal=_generarPasswordTemporal();
        const uIdx=(platDB.users||[]).findIndex(x=>x.u===user.u);
        if(uIdx!==-1){
          platDB.users[uIdx].p=await _hashPassword(passTemporal);
          _savePlatDBQuiet(plat.sk,platDB);
        }
        // Enviar correo real
        const resultado=await _enviarCorreoRecuperacion(email,user.n||user.u,user.u,passTemporal,user.r,plat.nombre);
        if(resultado.ok){
...
function htmlActividadesEstudiante(est){
  ...
  if(!acts.length) return _htmlEstadoVacio('👨‍🏫','No hay actividades asignadas por el docente.');
  const now=new Date();
  // Agrupar por período
  const grupos={};
...
function htmlQuizzesEstudiante(est){
  ...
  if(!evals.length) return _htmlEstadoVacio('📊','No hay evaluaciones/quizzes activos.');
  const now=new Date();
  return evals.map(ev=>{
  ...
  function renderQ(){
    ...
    ov.innerHTML=`<div style="background:#fff;border-radius:16px;padding:26px;max-width:620px;width:100%;max-height:90vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,0.4)">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-top:16px;flex-wrap:wrap;gap:8px">
        ...
        <div class="flex-gap">
          ${pregIdx>0?`<button class="btn" style="background:#eee;color:var(--text-secondary)" onclick="window._qNav(-1)">← Anterior</button>`:''}
  ...
  window._qFin=function(){
    ...
    const _revHtml=ev.preguntas.map((p,i)=>{
      ...
      return `<div style="border:2px solid ${bColor};border-radius:10px;padding:12px 14px;margin-bottom:10px;background:${bgColor};text-align:left">
        <div style="font-size:0.88rem;font-weight:bold;color:var(--text-secondary);margin-bottom:6px">${icon} ${i+1}. ${p.p}</div>
    ...
    ov.innerHTML=`<div style="background:#fff;border-radius:16px;padding:28px;max-width:580px;width:100%;max-height:92vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,0.4)">
      ...
      <h4 style="color:var(--text-secondary);font-size:0.92rem;margin:0 0 10px;border-bottom:1px solid #eee;padding-bottom:6px">📋 Revisión de respuestas</h4>
      ${_revHtml}
...
function htmlEvalDocenteEstudiante(est){
  ...
  if(!pendientes.length) return `<div class="card" style="border-left:4px solid #f39c12"><h4 class="card-title">⭐ Evaluación Docentes</h4>${_htmlEstadoVacio('👨‍🏫','Ya respondiste la evaluación de tus docentes. ¡Gracias!')}</div>`;
...
function htmlEvalDocenteAdmin(){
  ...
  <div class="card">
    ...
    }).join(''):`${_htmlEstadoVacio('📮','No hay encuestas creadas.')}`}
  </div>`;
}
let _evdPregs=[];
...
// ============================================================
// MIGRACIÓN DE NOTAS AL TRASLADAR UN ESTUDIANTE DE GRADO
// Las notas se guardan bajo el id de la "carga" (asignación docente+
// materia+grado), que es distinto en cada grado — por eso, cambiar
// solo el grado del estudiante no bastaba: las notas seguían
// existiendo pero quedaban "huérfanas" bajo el id de la materia del
// grado ANTERIOR, invisibles en la Planilla del grado nuevo.
// Esta función busca, para cada materia con notas en el grado de
// origen, la materia EQUIVALENTE (por nombre) en el grado destino, y
// traslada las notas a esa nueva "carga". Si el grado destino no
// tiene esa materia (ej. una modalidad distinta), la nota queda
// donde estaba — no se borra nada — y se reporta para que el
// administrador lo sepa.
// Nota: descriptores (banco de textos por grado/nivel, no por
// estudiante), observador (vive dentro del propio registro del
// estudiante) y actas (documentos de texto libre) NO necesitan esta
// migración — ya se conservan automáticamente al cambiar el grado.
// ============================================================
...
function _migrarNotasTraslado(d,estId,gradOrigen,gradDest){
  ...
  if(idx<0) return [];
  const e=d.ests[idx];
  const cargaOrigen=(d.carga||[]).filter(function(c){return c.g===gradOrigen;});
  const cargaDest=(d.carga||[]).filter(function(c){return c.g===gradDest;});
...
async function _ejecutarTraslado1Real(estId){
  ...
  let materiasSinEquivalente=[];
  updDB(d=>{
    materiasSinEquivalente=_migrarNotasTraslado(d,estId,gradOrigen,gradDest);
    _registrarCambioMatricula(d,{estId,estNombre:fmtNombreEst(e),tipo:'traslado_grado',detalle:gradOrigen+' → '+gradDest});
    return d;
  });
  document.getElementById('_trasladoModal')?.remove();
  if(materiasSinEquivalente.length){
    await customAlert('✅ '+fmtNombreEst(e)+' trasladado(a) de '+gradOrigen+' → '+gradDest+'. Notas migradas.\n\n⚠️ El grado destino no tiene estas materias, así que esas notas puntuales no se pudieron trasladar automáticamente (siguen existiendo, solo no se ven en el nuevo grado): '+materiasSinEquivalente.join(', '));
  }else{
    _showToast(`✅ ${fmtNombreEst(e)} trasladado(a) de ${gradOrigen} → ${gradDest}. Notas migradas al nuevo grado.`,'#16a085');
  }
  renderApp();
}
function _ejecutarTraslado1(estId){
  _ejecutarTraslado1Real(estId);
}
  ...
  ov.innerHTML=`<div style="background:#fff;border-radius:14px;width:100%;max-width:680px;box-shadow:0 8px 40px rgba(0,0,0,0.35);overflow:hidden;margin:auto">
    ...
    <div style="padding:20px">
      ...
      <div style="background:#fff3cd;border:1px solid #ffc107;border-radius:8px;padding:10px 14px;font-size:0.82rem;color:#856404;margin:14px 0">
        ✅ Las notas de todas las materias se migran automáticamente a la materia equivalente del grado destino (por nombre). Si el grado destino no tiene alguna de esas materias, esa nota puntual no se pierde, pero no se podrá ver hasta que se resuelva manualmente.
      </div>
      <div style="display:flex;gap:10px">
...
async function _ejecutarTrasladoMasivoReal(){
  ...
  let count=0;
  const materiasSinEquivalenteTotal=new Set();
  updDB(d=>{
    selIds.forEach(function(estId){
      const e=d.ests.find(function(x){return String(x.id)===String(estId);});
      if(!e) return;
      const sinEquiv=_migrarNotasTraslado(d,estId,gOrigen,gDest);
      sinEquiv.forEach(function(m){materiasSinEquivalenteTotal.add(m);});
      _registrarCambioMatricula(d,{estId,estNombre:fmtNombreEst(e),tipo:'traslado_grado',detalle:gOrigen+' → '+gDest});
      count++;
    });
    return d;
  });
  document.getElementById('_trasladoMasivoModal')?.remove();
  if(materiasSinEquivalenteTotal.size){
    await customAlert('✅ '+count+' estudiante(s) trasladado(s) de '+gOrigen+' → '+gDest+'. Notas migradas.\n\n⚠️ El grado destino no tiene estas materias, así que esas notas puntuales no se pudieron trasladar automáticamente (siguen existiendo, solo no se ven en el nuevo grado): '+[...materiasSinEquivalenteTotal].join(', '));
  }else{
    _showToast(`✅ ${count} estudiante(s) trasladado(s) de ${gOrigen} → ${gDest}. Notas migradas al nuevo grado.`,'#16a085');
  }
  renderApp();
}
