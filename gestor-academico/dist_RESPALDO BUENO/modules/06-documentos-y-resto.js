...
async function enviarComunicadoGeneral(){
  ...
  try{
    ...
    const r=await fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:tipo,actor,message:mensajeCompleto,meta})});
    ok=r.ok;
...
function enviarAusentismo(){
  ...
  try{fetch('/api/inetis/notify',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:'permiso-laboral',actor:sesion.n,message:sesion.n+' envió una solicitud de permiso laboral (H03.03.F01). Tipos: '+tipos.join(', ')+'.',meta:solicitud})}).catch(()=>{});}catch(e){}
