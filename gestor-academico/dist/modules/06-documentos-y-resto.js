...
function cargarNotificaciones(){
  ...
  fetch(API_BASE+'/api/inetis/notifications?sk='+encodeURIComponent(_skActual()||'')+'&limit=100').then(r=>r.json()).then(j=>{
    ...
    if(!list.length){wrap.innerHTML='<p class="empty">Sin notificaciones aún.</p>';return;}
  ...
  try{
    ...
    cont.innerHTML=avisos.map(n=>{
      ...
      return `<div style="border-left:4px solid #1e8449;background:#eafaf1;padding:10px 14px;border-radius:0 8px 8px 0;margin-bottom:10px">
        ...
        <p style="margin:0 0 4px;font-weight:700;font-size:0.87rem;color:#222">${m.asunto||'(sin asunto)'}</p>
        ${cuerpo?`<p style="margin:0 0 4px;font-size:0.83rem;color:#333;line-height:1.5;white-space:pre-wrap">${cuerpo}</p>`:''}
  ...
  try{
    ...
    histItems.innerHTML=mios.map(n=>{
      ...
      return `<div style="border-left:3px solid #1e8449;padding:7px 12px;margin-bottom:8px;background:#fff;border-radius:0 6px 6px 0">
        ...
        <p style="margin:4px 0 0;font-size:0.84rem;color:#333;font-weight:600">${m.asunto||'(sin asunto)'}</p>
      </div>`;
    }).join('');
  }catch(e){
...
function htmlVerCredenciales(){
  ...
  <div class="card">
    ...
    <p style="font-size:0.75rem;color:#888;margin-top:6px">🔒 Por seguridad, las contraseñas de docentes se guardan cifradas y ya no pueden mostrarse en texto — use "🔑 Restablecer" para definir una nueva si el docente la olvidó.</p>`:'<p class="empty">Sin docentes registrados.</p>'}
  </div>
  <div class="card" style="margin-top:14px">
  ...
  <div style="display:grid;grid-template-columns:1.3fr 1fr;gap:16px" class="cal-grid-resp">
    <div class="card">
      ...
      ${proximos.length?proximos.map(function(ev){
        ...
        +'</div>';
      }).join(''):'<p class="empty">No hay eventos próximos registrados.</p>'}
    </div>
    <div class="card">
...
function _htmlComparativoAniosResultado(anioA,anioB,hist,anioActivo){
  ...
  if(!datosA||!datosB) return '<p class="empty">No hay datos completos para uno de los años seleccionados.</p>';
  ...
  if(!gradosTodos.length) return '<p class="empty">Ninguno de los dos años tiene grados con notas registradas para comparar.</p>';
