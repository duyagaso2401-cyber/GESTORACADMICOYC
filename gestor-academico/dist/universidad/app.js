...
(function(){
  ...
  function render(){
    ...
    document.getElementById('app').innerHTML=
      ...
      // El ícono de sincronización (☁️) va AQUÍ, en la barra superior junto al
      // botón de cerrar sesión — antes no existía ningún #_syncChip en esta
      // página, así que 07-sync-engine.js creaba uno flotante en la esquina
      // inferior derecha (position:fixed;bottom:20px;right:20px), exactamente
      // donde también flota el avatar del Asistente IA (mismo bottom/right),
      // quedando los dos pegados. Al declararlo aquí, en el flujo normal del
      // topbar, sync-engine.js lo detecta (getElementById('_syncChip')) y
      // reutiliza este en vez de crear uno flotante — discreto, sin invadir
      // ni chocar con el asistente ni con nada más de la pantalla.
      +'<div class="topbar-right"><span id="_syncChip" class="sync-engine-chip sync-engine-chip--inline" title="Sincronización con el servidor">☁️</span><button class="btn gris" style="padding:7px 14px;font-size:0.78rem" onclick="_univCerrarSesion()">Cerrar sesión</button></div>'
      +'</div>'
